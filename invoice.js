...
function getInvoiceCSS() {
    return `
        ...
        .inv-company { flex: 1; }
        .inv-company h1 {
            font-size: 29px; color: #2eaadc; margin: 0 0 10px 0;
            text-transform: uppercase; font-weight: bold; line-height: 1;
        }
        .inv-company p {
            margin: 4px 0; font-size: 12px; color: #787774; line-height: 1.4;
        }
        .inv-logo-sec {
        ...
        .inv-type {
            font-size: 26px; font-weight: bold; color: #94cce6; text-transform: uppercase; /* Soft Primary Blue */
        }
        .inv-meta-grid {
            display: grid; grid-template-columns: 1.2fr 0.8fr; gap: 20px; margin-bottom: 30px;
        }
        .inv-section-title {
            font-size: 13px; color: #2eaadc; font-weight: bold; margin: 0 0 8px 0; text-transform: uppercase; /* Primary Blue */
        }
        .inv-client-name {
            font-size: 15px; font-weight: normal; color: #37352f; margin: 0 0 4px 0;
        }
        .inv-sub { font-size: 12px; color: #787774; margin: 0; }
        .inv-details-row {
            display: flex; justify-content: space-between; font-size: 12px; color: #787774; margin-bottom: 6px;
        }
        .inv-details-val { text-align: right; color: #37352f; }
        .inv-table { width: 100%; border-collapse: collapse; margin-bottom: 20px; }
        .inv-table th {
            background: rgba(46, 170, 220, 0.08); padding: 8px 6px; text-align: left; font-size: 12px; /* Soft Blue BG */
            font-weight: bold; color: #37352f; border-bottom: 1px solid rgba(46, 170, 220, 0.2);
        }
        .inv-table td {
            padding: 8px 6px; font-size: 12px; color: #37352f;
            border-bottom: 1px solid #f0f0f0; vertical-align: middle;
        }
        .col-num { width: 8%; text-align: center; color: #787774; }
        .col-desc { width: 45%; }
        ...
        .inv-total-row {
            ...
            font-size: 16px; font-weight: bold; color: #2eaadc;
        }
        .inv-footer { margin-top: auto; padding-top: 30px; }
        .inv-footer-line { border-top: 2px solid #2eaadc; margin-bottom: 15px; width: 40px; }
        .inv-footer h4 { font-size: 13px; margin: 0 0 10px 0; color: #2eaadc; text-transform: uppercase; /* Primary Blue */ }
        .inv-banks { display: grid; grid-template-columns: 1fr 1fr; gap: 15px 40px; }
        .inv-bank-item { font-size: 11px; line-height: 1.4; }
        .inv-bank-item strong { display: block; color: #37352f; font-weight: bold; }
        .inv-bank-item span { color: #787774; }
        .inv-thankyou { text-align: center; font-size: 11px; color: #9b9a97; margin-top: 30px; }
    `;
}
...
export async function generateInvoice(pnrList, type = 'Invoice', dateStr = null, forcedMode = 'auto') {
    ...
    // Colors
    const PRIMARY_COLOR = [46, 170, 220]; // #2eaadc
    const TEXT_COLOR = [55, 53, 47]; // #37352f
    const SUBTEXT_COLOR = [120, 119, 116]; // #787774
    const TABLE_HEAD_BG = [240, 250, 255]; // Soft Blue BG for Table Header
    const TABLE_BORDER_COLOR = [191, 219, 254]; // Soft Blue Border
    const SOFT_PRIMARY_COLOR = [148, 204, 230]; // Lighter Primary for Big Labels
    ...
    invoiceGroups.forEach((group, groupIndex) => {
        ...
        doc.setFont("helvetica", "bold");
        doc.setFontSize(20);
        doc.setTextColor(...SOFT_PRIMARY_COLOR); /* Use Soft Primary Blue for Doc Type Label */
        ...
        doc.setFont("helvetica", "bold");
        doc.setFontSize(10);
        doc.setTextColor(...PRIMARY_COLOR); /* Use Primary Blue for Section Headers */
